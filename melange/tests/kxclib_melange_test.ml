...
module Jcsnafi_test : sig
  ...
  let test_unparse_jcsnafi () =
    ...
    [
      ...
      case (`num 0.) {|0|};
      case (`num (+0.)) {|0|};
      case_exn (`num (pred min_fi_float)) (Invalid_argument "Number cannot be safely encoded with Json_JCSnafi (encountering: -4503599627370497.000000)");
      case_exn (`num (succ max_fi_float)) (Invalid_argument "Number cannot be safely encoded with Json_JCSnafi (encountering: 4503599627370495.500000)");
  ...
  let test_is_encodable () =
    ...
    [
      ...
      case (`num 0.) true;
      case (`num (+0.)) true;
      case (`num (pred min_fi_float)) false;
      case (`num (succ max_fi_float)) false;
      case (`num (-1.5)) false;
      case (`num 4.8) false;
  ...
  let test_is_encodable_num () =
    ...
    [
      ...
      case (0.) true;
      case (+0.) true;
      case (pred min_fi_float) false;
      case (succ max_fi_float) false;
      case (-1.5) false;
      case 4.8 false;
    ] |> fold_promise
  ...
  let test_compare_field_name_rfc8785 () =
    ...
    |] in
    let sorted = Array.copy input in
    sorted |> Array.sort (fun (k1, _) (k2, _) ->
        Json_JCSnafi.compare_field_name k1 k2);
    (expect & sorted |> Array.map snd)
    |> Fn.flip to_strict_equal expected;
    Promise_io.return ()
end
